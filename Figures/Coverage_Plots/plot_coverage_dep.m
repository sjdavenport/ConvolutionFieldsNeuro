...
load([ncfloc,'Figures/Coverage_plots/' RSfolder, '_coverage5000_5']);
...
coverage_types = {'finelat', 'lat', 'conv'};
...
for I = 1:length(nsubj_vec)
    ...
    for do2tail = [0,1]
        ...
        for do_gauss = gauss_set{I}
            ...
            legend('finelat', 'lat', 'conv')
            
            filename = [gtindex, '_nsubj_', num2str(nsubj)];
            export_fig([ncfloc, 'Figures/Coverage_Plots/R2Block/', filename, '_EC0.pdf'], '-transparent')
        end
    end
end